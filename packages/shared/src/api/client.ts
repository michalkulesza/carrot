...
export const createApiClient = (config: ApiClientConfig) => {
  ...
  const fetchPublicRecipe = async (token: string): Promise<PublicRecipeOut> => {
    const res = await rawFetch(
      `${baseUrl}/api/public/recipes/${encodeURIComponent(token)}`,
      { credentials },
      "fetchPublicRecipe",
    );
  ...
  const addPublicRecipeToLibrary = async (
    token: string,
    householdId: string,
  ): Promise<RecipeOut> => {
    const res = await apiFetch(
      `/api/public/recipes/${encodeURIComponent(token)}/add-to-library`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const setRecipeHouseholds = async (
    id: string,
    householdIds: string[],
  ): Promise<RecipeOut> => {
  ...
  const removeRecipeFromHousehold = async (
    ...
    const res = await apiFetch(`/api/recipes/${id}/households/${householdId}`, {
      method: "DELETE",
    });
  ...
  const toggleFavourite = async (
    ...
    const res = await apiFetch(`/api/recipes/${recipeId}/favourite`, {
      method: "POST",
    });
  ...
  const uploadThumbnail = async (
    ...
    form.append("file", file);
    const res = await apiFetch(
      `/api/images/thumbnail?recipe_id=${encodeURIComponent(recipeId)}`,
      {
        method: "POST",
        body: form,
      },
  ...
  const addTagToRecipe = async (
    recipeId: string,
    tagId: string,
  ): Promise<void> => {
    const res = await apiFetch(`/api/recipes/${recipeId}/tags/${tagId}`, {
      method: "POST",
    });
    if (!res.ok) throw new Error("Failed to add tag");
  };

  const removeTagFromRecipe = async (
    recipeId: string,
    tagId: string,
  ): Promise<void> => {
    const res = await apiFetch(`/api/recipes/${recipeId}/tags/${tagId}`, {
  ...
  const getNextMealPlanEntry = async (
    from: string,
  ): Promise<MealPlanEntry | null> => {
    const res = await apiFetch(
      `/api/meal-plan/next?from=${encodeURIComponent(from)}`,
    );
  ...
  const leaveHousehold = async (id: string): Promise<void> => {
    const res = await apiFetch(`/api/households/${id}/leave`, {
      method: "POST",
    });
  ...
  const joinHouseholdByCode = async (
    code: string,
  ): Promise<{ active_household_id: string }> => {
  ...
  const rotateInviteCode = async (
    ...
    const res = await apiFetch(`/api/households/${householdId}/rotate-code`, {
      method: "POST",
    });
  ...
  const acceptInvitation = async (id: string): Promise<void> => {
    const res = await apiFetch(`/api/invitations/${id}/accept`, {
      method: "POST",
    });
  ...
  const declineInvitation = async (id: string): Promise<void> => {
    const res = await apiFetch(`/api/invitations/${id}/decline`, {
      method: "POST",
    });
    if (!res.ok) throw new Error("Failed to decline invitation");
  };

  const listHouseholdLeaveNotifications = async (): Promise<
    HouseholdLeaveNotificationOut[]
  > => {
  ...
  const dismissHouseholdLeaveNotification = async (
    id: string,
  ): Promise<void> => {
    const res = await apiFetch(
      `/api/household-leave-notifications/${id}/dismiss`,
      { method: "POST" },
    );
  ...
  const requestSignupCode = async (email: string): Promise<void> => {
    const res = await rawFetch(
      `${baseUrl}/api/auth/request-signup-code`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email }),
  ...
  const verifySignupCode = async (
    ...
    const res = await rawFetch(
      `${baseUrl}/api/auth/verify-signup-code`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const completeSignup = async (
    ...
    const res = await rawFetch(
      `${baseUrl}/api/auth/complete-signup`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const addShoppingListItems = async (
  ...
  const reorderShoppingList = async (
    category_orders: ShoppingCategoryOrders,
  ): Promise<void> => {
  ...
  const clearCompletedShoppingList = async (): Promise<void> => {
    const res = await apiFetch("/api/shopping-list/completed", {
      method: "DELETE",
    });
  ...
  const enqueueImportJob = async (
    data: ImportJobEnqueue,
  ...
  const retryImportJob = async (id: string): Promise<ImportJob> => {
    const res = await apiFetch(`/api/imports/jobs/${id}/retry`, {
      method: "POST",
    });
  ...
  const dismissImportJob = async (id: string): Promise<void> => {
    const res = await apiFetch(`/api/imports/jobs/${id}/dismiss`, {
      method: "POST",
    });
  ...
  const unregisterDevice = async (installationId: string): Promise<void> => {
    const res = await apiFetch(
      `/api/imports/devices/${encodeURIComponent(installationId)}`,
      { method: "DELETE" },
    );
      ...
      .then(async (res) => {
        ...
        while (!stopped) {
          ...
          for (const message of messages) {
            ...
            try {
              if (type === "import_jobs.snapshot")
                onSnapshot(JSON.parse(data) as ImportJobsSnapshot);
