          ...
          <div key={job.id} className="rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3 flex items-center gap-3">
            ...
            {job.status === 'failed' ? (
              <div className="flex gap-2">
                <Button size="sm" variant="secondary" onPress={() => void api.retryImportJob(job.id)}>{t('importJobs.retry')}</Button>
                <Button size="sm" variant="tertiary" onPress={() => void api.dismissImportJob(job.id)}>{t('importJobs.dismiss')}</Button>
              </div>
            ) : (
              <Button size="sm" variant="tertiary" onPress={() => void api.cancelImportJob(job.id)}>{t('importJobs.cancel')}</Button>
            )}
          </div>
        )
      })}
