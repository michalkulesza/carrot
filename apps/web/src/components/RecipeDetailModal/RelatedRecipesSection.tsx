...
const RelatedRecipesSection = ({ recipeId, onOpen }: { recipeId: string; onOpen: (id: string) => void }) => {
  ...
  return <section className="mb-5"><p className="text-xs font-semibold uppercase text-zinc-400 mb-2">{t('relatedRecipes.title')}</p>
    <div className="flex gap-3 overflow-x-auto pb-1">{relatedRecipes.map((recipe) => <div key={recipe.id} className="w-28 shrink-0 relative"><button type="button" onClick={() => onOpen(recipe.id)} className="w-full text-left"><NetworkImage src={proxyUrl(recipe.thumbnail_url)} className="h-16 w-28 rounded-lg" alt={recipe.title} /><p className="truncate text-xs mt-1">{recipe.title}</p></button><button type="button" disabled={save.isPending} onClick={() => void save.mutateAsync(relatedRecipes.filter((item) => item.id !== recipe.id).map((item) => item.id))} className="absolute right-1 top-1 rounded-full bg-black/50 text-white p-1 disabled:opacity-50"><X size={12} /></button></div>)}
